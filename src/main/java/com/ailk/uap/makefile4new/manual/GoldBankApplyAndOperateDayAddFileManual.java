...
/**
 * 该程序仅根据配置文件中的时间信息来生成指定的金库申请和审批日增量文件文件
 * @author jinlin
 *  type=SMJKR,SMJKA
 */
public class GoldBankApplyAndOperateDayAddFileManual implements
	GoldSceneOperateDayAddFileSQL {
    private static final Logger log = Logger
	    .getLogger(GoldBankApplyAndOperateDayAddFileManual.class);
	...
	a4GoldDayFileBuffer.append("<province>" + prov_code + "</province>"
		+ "\r\n");
	a4GoldDayFileBuffer.append("<createtime>" + createTime.replace(" ","T")
		+ "</createtime>" + "\r\n");
    ...
    private static void writeA4AppAcctDayAddFileBufferToTempFile(
	...
	while (rs.next()) {
	    ...
	    a4GoldDayFileBuffer.append("<requesttime>"
		    ...
		    + "</requesttime>" + "\r\n");
	    a4GoldDayFileBuffer.append("<requestinfo>"
		    + rs.getString("requestinfo").trim().replace("&", "")  + "</requestinfo>"
		    + "\r\n");
